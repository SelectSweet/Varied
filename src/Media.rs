...
pub async fn ViewMedia(
    ...
    if MediaType != "Note" && MediaType != "Text" {
        ...
        let Poster: Vec<Value> = ViewMedia["poster_storagepathorurl"]
            .as_array()
            .unwrap()
            .to_vec();
        for l in Media {
            MediaUrl.push(json!(
                "http://localhost:8000".to_owned()
                    + "/api/media/file/"
                    + &l.to_owned().as_str().unwrap().replace("Cache/", "")
            ));
        }
...
pub async fn UpdateDetails(
    ...
    if data.PublicId == "" {
        return Ok("A Media ID is required".to_string());
    }
    ...
    if token.0.authorize(&token.1).is_ok() {
        ...
        return Ok("Media has been Updated".to_string());
    } else {
